    ...
    <>
      ...
      <section className="lesson">
        ...
        <InfoBox kind="key" title="本质">
          「枚举子集」<M>{'\\big(O(3^n)\\big)'}</M> 与「位掩码 + 附加维计数」是状压的两把<strong>通用扳手</strong>：前者应对「把集合劈成两块」的划分型转移；后者把 <M>{'dp[mask]'}</M> 升成 <M>{'dp[mask][\\text{附加量}]'}</M>，让状压能数方案、能带取模、能挂任意可累加的辅助信息。它们不是新模型，而是嫁接在前几类骨架上的技巧。
        </InfoBox>
      </section>
