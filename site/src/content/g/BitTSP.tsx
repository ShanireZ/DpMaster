    ...
    <>
      <section className="lesson">
        ...
        <div className="prose">
          ...
          <p>
            但仔细想：走到某一步时，<strong>接下来怎么走最优，只跟两件事有关</strong>——「<strong>已经走过了哪些点</strong>」和「<strong>此刻站在哪个点</strong>」。至于这些点是按什么顺序走到的，对未来毫无影响。这就把 <M>{'n!'}</M> 条路径，坍缩成了 <M>{'(\\text{已访问集合})\\times(\\text{当前点})'}</M> 这么多状态。
          </p>
        </div>
      ...
      <section className="lesson">
        ...
        <div className="prose">
          ...
          <p>
            边界：<M>{'dp[\\{0\\}][0]=0'}</M>（从点 0 出发，只到过点 0，路程 0）。答案：走遍全集后停在终点，<M>{'dp[(1{<}{<}n)-1][\\,\\text{终点}\\,]'}</M>。
