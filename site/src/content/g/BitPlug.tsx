  ...
  return (
    <>
      ...
      <section className="lesson">
        ...
        <div className="prose">
          <p>
            由于轮廓线状态数远小于 <M>{'2^{2m}'}</M> 的上界（合法括号序列稀疏），通常用<strong>哈希表</strong>滚动存「状态 → 方案数」。换行时把整条轮廓线<strong>左移一格</strong>（最高位插头清零），用位运算实现：
          </p>
          <MB>{'state \\leftarrow state\\,{<}{<}\\,2\\quad(\\text{每个插头 2 位，换行整体左移})'}</MB>
        </div>
