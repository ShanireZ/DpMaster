    ...
    <>
      ...
      <section className="lesson">
        ...
        <div className="prose">
          ...
          <p>
            而 LIS 有 <M>{'O(n\\log n)'}</M> 的<Link to="/part/a/lis" style={{ color: 'var(--accent-2)' }}>贪心 + 二分</Link>解法（维护 <M>{'tails'}</M>、<M>{'\\texttt{lower\\_bound}'}</M> 替换）。绕这一圈，排列 LCS 就从 <M>{'O(n^2)'}</M> 降到了 <strong><M>{'O(n\\log n)'}</M></strong>。
          </p>
        </div>
        <div className="pointer-cue">
          <MousePointerClick size={18} />
      ...
      <nav className="type-nav">
        ...
        <Link to="/part/a/edit" className="next">
