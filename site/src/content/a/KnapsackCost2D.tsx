      ...
      <section className="lesson">
        ...
        <div className="prose">
          ...
          <p>
            ...
            <Link to="/part/a/01" style={{ color: 'var(--accent-2)' }}>「必须倒序」</Link>完全一致：倒序时 <M>{'dp[x-a][y-b]'}</M> 用的是<strong>本件尚未装入</strong>的旧值，才能保证每件至多取一次。三层循环的骨架是「逐件 → 费用1 倒序 → 费用2 倒序」：
          </p>
          <pre
            className="mono"
            style={{
    ...
    for y = B downto b:      // ★费用2 维倒序
      dp[x][y] = max( dp[x][y], dp[x − a][y − b] + v )`}
        ...
        <InfoBox kind="warn" title="记死：两维都倒序，缺一维就退化成完全背包">
          ...
          <Link to="/part/a/complete" style={{ color: 'var(--accent-2)' }}>完全背包</Link>那样「同一件被反复装入」，答案偏大。若题目本就允许每件取无限次（二维费用的<strong>完全型</strong>），才把两维都改成正序。
        </InfoBox>
      </section>
