  ...
  return (
    <>
      <section className="lesson">
        ...
        <div className="prose">
          <p>
            前三类背包各自只管一种「件数」：<Link to="/part/b/01" style={{ color: 'var(--accent-2)' }}>01 背包</Link>每种<strong>恰一件</strong>、
            <Link to="/part/b/complete" style={{ color: 'var(--accent-2)' }}>完全背包</Link>每种<strong>无限件</strong>、
            <Link to="/part/b/multiple" style={{ color: 'var(--accent-2)' }}>多重背包</Link>每种<strong>有限 <M>{'m'}</M> 件</strong>。
...
{`for 每件物品 (w, v, p):        // p = 件数：0 表示无限
    if p == 0:                 // 无限 → 完全，正序
        ...
        <InfoBox kind="warn" title="常见陷阱 · 别把方向记反 / 别忘拆多重">
          ...
          其二，<strong>多重件忘了二进制拆分</strong>，直接当完全（正序）会超取、直接当 01（一个包倒序）会漏取。拿不准时回看 <Link to="/part/b/01" style={{ color: 'var(--accent-2)' }}>01</Link> / <Link to="/part/b/complete" style={{ color: 'var(--accent-2)' }}>完全</Link> / <Link to="/part/b/multiple" style={{ color: 'var(--accent-2)' }}>多重</Link> 三页的方向依据。
        </InfoBox>
      </section>
      ...
      <div className="pointer-cue">
        <Gamepad2 size={18} />
        想找手感？到 <Link to="/part/b" style={{ color: 'var(--accent-1)', fontWeight: 600 }}>B 部分页的「装包大师」</Link>再挑一挑——它虽按 01 规则，但「在约束下凑最大价值」的直觉，与混合背包里逐件分派的思路完全相通。
      </div>

      <nav className="type-nav">
        <Link to="/part/b/group">
          <span className="dir">← 上一类型</span>
          <span className="nm">
            <ArrowLeft size={15} style={{ verticalAlign: '-2px' }} /> 分组背包
          </span>
        </Link>
        <Link to="/part/b/cost2d" className="next">
