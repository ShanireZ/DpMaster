  ...
  return (
    <div className="about">
      ...
      <section className="about-block about-policy" id="privacy">
        ...
        <p>
          反馈表单只在你主动提交时发送数据。提交内容包括反馈类型、具体描述、当前页面名称与路径、完整网址、
          提交时间、浏览器与设备标识、视口与屏幕尺寸、语言和时区；接收端还会从请求头记录来源 IP。联系方式可以留空匿名。
        </p>
        <p>
          提交成功仅在维护通道确认接收后显示，并提供回执编号；通道不可用时页面会明确提示失败，不会把“仅写入日志”冒充送达。
          两个生产域名均使用 Cloudflare Web Analytics 汇总页面访问与性能趋势；站内 Web Vitals 与使用事件只记录页面路径、
          事件类别和性能数值，不采集姓名、联系方式或输入内容。
        </p>
        <p>
