...
//#region src/algorithms/bitmask-board/internal.ts
function isLegalKingRow(mask) {
	return (mask & mask << 1) === 0;
}
function areCompatibleKingRows(mask, previous) {
...
function legalKingRows(size) {
	const rows = [];
	for (let mask = 0; mask < 1 << size; mask++) if (isLegalKingRow(mask)) rows.push(mask);
	return rows;
}
function findLayout(size, kings, masks, counts) {
	const rows = [];
	const visit = (row, placed) => {
		if (placed === kings) {
...
function executeKingsBoard(size, kings, emit) {
	...
	let table = Array.from({ length: kings + 1 }, () => Array(masks.length).fill(0));
	for (let index = 0; index < masks.length; index++) if (counts[index] <= kings) table[counts[index]][index] = 1;
	for (let row = 2; row <= size; row++) {
		const next = Array.from({ length: kings + 1 }, () => Array(masks.length).fill(0));
		for (let maskIndex = 0; maskIndex < masks.length; maskIndex++) for (let placed = counts[maskIndex]; placed <= kings; placed++) for (let previousIndex = 0; previousIndex < masks.length; previousIndex++) {
	...
	return {
		total: table[kings].reduce((sum, value) => sum + value, 0),
		layout: findLayout(size, kings, masks, counts),
		masks,
		counts,
		table
	};
...
//#endregion
//#region src/algorithms/bitmask-board/index.ts
function solveKingsBoard(size, kings) {
	return executeKingsBoard(size, kings, ignoreEvents);
}
//#endregion
export { countBits as n, solveKingsBoard as t };
