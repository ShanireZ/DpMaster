...
The Worker preserves the validator supplied by Static Assets for each representation. At the final edge, Cloudflare automatically injects Web Analytics and JavaScript Detections into HTML; Cloudflare documents that JSD removes ETags from injected HTML responses. The client-facing HTML response therefore deliberately has no validator while those established edge features remain enabled. Markdown is not transformed by that HTML injection and retains its independent strong asset validator.
