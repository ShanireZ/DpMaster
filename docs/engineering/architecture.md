...
HTML deliberately weakens the Static Assets strong ETag before returning it. Cloudflare may content-encode the HTML body after Worker execution; a weak validator remains valid across that byte transformation and survives at the final edge, while Markdown retains its independent asset validator.
