...
- 分类主雕塑、课程图集与 Demo Hero 均保留独立高分辨率源图；生产交付统一使用透明 AVIF，并按实际展示尺寸压缩，不把生成母版直接发送到客户端。
...
- 14 门代表课继续作为家族构图的评审归档；实现范围则覆盖全部 37 门课程，每课首个真实 Demo 都有按该课机制独立生成、独立文件的静态 Hero。
- Demo Hero 通过共享 `DemoSculptureHero` 接入，只承担装饰，不读取参数、求解结果或播放帧；禁止复制另一课程资产、图集裁片或把动态数据烘焙进 Hero。
...
- 37 门静态 Hero 保持一课一资产、纯装饰且与动态数据解耦；14 门代表课仍保留为视觉方向评审样本。
- 当前路由只加载所属家族的艺术模块与资产。
