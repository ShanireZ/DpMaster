...
- 14 门代表课保留已拍板的独立高保真静态 Hero；A/complete 另有课程专用静态 Hero。它们均为纯装饰，不读取参数、播放帧或求解状态。其余课程依靠唯一页头图例与真实 Demo，不复制冗余装饰资产。
...
2. 14 门代表 Hero 必须保持独立资产、空替代文本、`aria-hidden`、无输入和无播放状态耦合。
3. 代表课程需持续断言真实交互产生状态变化；高密度课程 A/cost2d、B/edit、C/tree、D/matpow、E/inout、F/knapsack、G/tsp、G/plug 保留专项回归。
