...
`.cc` alerts ride the same path: the `.cc` browser posts feedback and analytics events straight to `https://dp.betaoi.cn/api/*` because the Cloudflare datacenter egress to China infrastructure (DingTalk and EdgeOne alike) fails TLS with 525 while ordinary overseas hosts reach both. EdgeOne allows the first-party `https://dp.betaoi.cc` origin via a CORS allowlist (preflight included) and rejects every other cross-origin request. Feedback then follows the normal `.cn` webhook chain; `client_error` / `feedback_failed` analytics alerts are delivered by the `.cn` runtime, and the submitter IP/limiter key come from the EdgeOne platform.

The server-side relay (`FEEDBACK_RELAY_URL` + `x-dp-relay-secret` / `x-dp-client-ip` / `x-dp-relay-kind: alert`) remains as a fallback for future transports, but the Cloudflare egress currently cannot reach the `.cn` host, so it is not part of the active chain.
